...
function timeSpanToString(startDate, endDate) {
  ...
  return `${[hours, minutes, seconds].map((v) => (`0${v}`).slice(-2)).join(':')}.${(`00${milliseconds}`).slice(-3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const diff = Math.abs(ha - ma) % 360;
  const minAngle = Math.abs(Math.min(360 - diff, diff));
